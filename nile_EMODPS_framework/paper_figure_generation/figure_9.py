...
for scen in tqdm(range(5000)):
    ...
    for i, obj in enumerate(outcomes.iloc[:, :-2].columns):
        best = scen_df.sort_values(by=[obj], ascending=(directions[i] == "min")).iloc[
            0
        ][obj]
...
for i, obj in enumerate(outcomes.iloc[:, :-2].columns):
    ...
    ax[i // 2][i % 2].set_title(
        f"{letters[i]}) {fancy_objective_names[obj]}", fontsize=18
    )

handles = [
    mpl.patches.Patch(facecolor=color, label=name)
    for name, color in policy_colors.items()
    if name
    not in ["All Solutions", "Best Egypt Irrigation", "Best Ethiopia Hydropower"]
]
